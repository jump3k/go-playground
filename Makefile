...
.PHONY: build
build: build_srs_hook build_rpc_server build_rpc_client build_stringsvc3 build_rtmpSvc
...
# build rtmpSvc
.PHONY: build_rtmpSvc
build_rtmpSvc: $(BINDIR)/rtmpSvc

$(BINDIR)/rtmpSvc: $(SRC)
	GO111MODULE=on go build $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $@ ./cmd/rtmpserver
